...
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">이사 견적리스트</h1>
	</div>
	<!-- /.col-lg-12 -->
</div>
<!-- /.row -->
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					<label>신청인</label>
					<input class="form-control" name='name' value='<c:out value="${estimate.name}"/>' readonly="readonly">
				</div>
				<div class="form-group">
					<label>휴대폰</label>
					<input class="form-control" name='phone' value='<c:out value="${estimate.phone}"/>' readonly="readonly">
				</div>
			</div>
			</div>
			<div class="row">
				...
				<div class="form-group">
					<label>공사 제목</label>
					<input class="form-control" name='title' value='<c:out value="${estimate.title}"/>' readonly="readonly">
				</div>
				<div class="form-group">
					<label>출발주소</label>
					<input class="form-control" name='departure' value='<c:out value="${estimate.departure}"/>' readonly="readonly">
				</div>
				<div class="form-group">
					<label>도착주소</label>
					<input class="form-control" name='arrive' value='<c:out value="${estimate.arrive}"/>' readonly="readonly">
				</div>
				<div class="form-group">
					<label>이사분류</label>
					<input class="form-control" name='move' value='<c:out value="${estimate.move}"/>' readonly="readonly">
				</div>
				<div class="form-group">
					<label>이사예정일</label>
					<input class="form-control" name='depdate' value='<fmt:formatDate pattern="yyyy-MM-dd"
							value="${estimate.depDate}" />' readonly="readonly">
				</div>
...
<%--       				<input type='hidden' id='replyer2' value="${pinfo.username}"> --%>
      				<!-- <input type='hidden' id='replyer2' value="">
...
<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/estireply.js"></script>

<script>
$(document).ready(function () {
	var enoValue = '<c:out value="${estimate.eno}"/>';
	var replyUL = $(".chat");
    showList(1);
    function showList(page){
    	console.log("show list " + page);
    	// 해당 게시물의 댓글을 가져오기 위함
    	replyService.getList({eno:enoValue,page: page|| 1 }, function(replyCnt, list) {
    ...
    var modalInputCompname = modal.find("input[name='compname']");
    var modalInputCompphone = modal.find("input[name='compphone']");
    var modalInputPrice = modal.find("input[name='price']");
    ...
    $("#RegisterBtn").on("click",function(e) {
    	var reply = { reply: $('#reply2').val(), replyer:$('#replyer2').val(), eno:enoValue };
	...
	modalRegisterBtn.on("click",function(e) {
    	var reply = { compname:modalInputCompname.val(),
    				  compphone:modalInputCompphone.val(),
    ...
    $(".chat").on("click", "li", function(e) {
    	...
    	replyService.get(rno, function(reply) {
    		modalInputCompname.val(reply.compname);
    		modalInputCompphone.val(reply.compphone);
    		modalInputPrice.val(reply.price);
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("button[data-oper='modify']").on("click",	function(e) {
			operForm.attr("action", "${pageContext.request.contextPath}/estimate/modify").submit();
		});
		$("button[data-oper='list']").on("click", function(e) {
			operForm.find("#eno").remove();
			operForm.attr("action", "${pageContext.request.contextPath}/estimate/estimatelist")
			operForm.submit();
		});
	});
</script>

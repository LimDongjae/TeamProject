...
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">견적 리스트</h1>
	</div>
	<!-- /.col-lg-12 -->
</div>
<!-- /.row -->
...
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<table class="table table-striped table-bordered table-hover">
					...
					<c:forEach items="${list}" var="estimate">
						<tr>
							<td><c:out value="${estimate.eno}" /></td>
							...
							value="${estimate.depDate}" /></td>
							<td><fmt:formatDate pattern="yyyy-MM-dd"
									value="${estimate.updateDate}" /></td>
							<td><c:out value="${estimate.status }"/></td>
						</tr>
					</c:forEach>
				</table>
				...
				<div class='row'>
					<div class="col-lg-12">
						<!-- 검색 부분 -->
						<form id='searchForm' action="${pageContext.request.contextPath}/estimate/estimatelist" method='get'>
							<select name='type'>
								<option value=""
			...
			<form id='actionForm' action="${pageContext.request.contextPath}/estimate/estimatelist" method='get'>
	...
	$(document).ready(function() {
		...
		function checkModal(result) {
			...
			if (parseInt(result) > 0) {
				$(".modal-body").html("게시글 " + parseInt(result) + " 번이 저장되었습니다.");
			}
			$("#myModal").modal("show");
		}
		...
		$("#regBtn").on("click", function() {
			self.location = "${pageContext.request.contextPath}/estimate/estimate";
		});
		...
		$(".move").on("click",function(e) {
			...
			actionForm.attr("action", "${pageContext.request.contextPath}/estimate/get");
			actionForm.submit();
		});
